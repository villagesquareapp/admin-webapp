...
const kpiCards = (k: IEchoOverview["kpis"]) => [
  { label: "Total echoes", value: k.total, icon: "solar:microphone-3-linear", tone: "bg-lightprimary text-primary" },
  { label: "Live now", value: k.live, icon: "solar:soundwave-linear", tone: "bg-lightsuccess text-success" },
  ...
  { label: "Listeners", value: k.total_listeners, icon: "solar:users-group-rounded-linear", tone: "bg-lightsecondary text-secondary" },
...
const Cover = ({ src }: { src?: string }) => (
  <div className="relative w-11 h-11 rounded-lg overflow-hidden bg-lightgray dark:bg-dark shrink-0">
    {src && <Image src={src} alt="" fill className="object-cover" />}
  </div>
);

const EchoRow = ({ e, metric }: { e: IEchoes; metric: "listeners" | "gifts" }) => (
  <Link href={`/dashboards/echoes/${e.uuid}`} className="flex items-center gap-3 py-2.5">
    <Cover src={e.cover} />
    <div className="min-w-0 flex-1">
      <p className="text-sm font-medium truncate">{e.title || "Untitled echo"}</p>
      <p className="text-xs text-darklink truncate">@{e.host?.username}</p>
    </div>
    {metric === "listeners" ? (
      <div className="text-right shrink-0">
        <p className="text-sm font-semibold tabular-nums">{fmt(e.total_listeners_ever || e.peak_listener_count)}</p>
        <p className="text-[11px] text-darklink">listeners</p>
      </div>
    ) : (
      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full capitalize shrink-0 ${ECHO_STATUS_TONE[e.status]}`}>
        {e.status}
      </span>
    )}
  </Link>
...
const EchoOverviewContent = ({ data }: { data: IEchoOverview | null }) => {
  if (!data) {
    return (
      <CardBox>
        <div className="py-16 text-center text-darklink">Echo overview is unavailable right now.</div>
      </CardBox>
    );
  }
  const { kpis, by_status, by_category, engagement, live_now, top_echoes, reported_echoes } = data;
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* engagement */}
      <CardBox>
        <h5 className="card-title mb-3">Engagement across all echoes</h5>
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {[
            { icon: "solar:users-group-rounded-linear", v: engagement.listeners, l: "Listeners", t: "bg-lightprimary text-primary" },
            ...
            { icon: "solar:emoji-funny-circle-linear", v: engagement.reactions, l: "Reactions", t: "bg-lightsecondary text-secondary" },
            { icon: "solar:chat-round-linear", v: engagement.chat, l: "Chat", t: "bg-lightinfo text-info" },
          ].map((s) => (
        ...
        <div className="col-span-12 lg:col-span-6">
          <CardBox>
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/dashboards/echoes/list?status=live" className="text-primary text-sm font-semibold">View all</Link>
            </div>
            {live_now.length ? (
              <div className="flex flex-col divide-y divide-border dark:divide-darkborder">
                {live_now.map((e) => <EchoRow key={e.uuid} e={e} metric="listeners" />)}
              </div>
            ) : (
              <div className="py-6 text-center text-sm text-darklink">No echoes are live right now.</div>
            )}
          </CardBox>
        </div>
        <div className="col-span-12 lg:col-span-6">
          <CardBox>
            ...
            {reported_echoes.length ? (
              <div className="flex flex-col divide-y divide-border dark:divide-darkborder">
                {reported_echoes.map((e) => (
                  <Link key={e.uuid} href={`/dashboards/echoes/${e.uuid}`} className="flex items-center gap-3 py-2.5">
                    <Cover src={e.cover} />
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium truncate">{e.title || "Untitled echo"}</p>
                      <p className="text-xs text-darklink truncate">@{e.host?.username}</p>
      ...
      <div className="grid grid-cols-12 gap-5">
        ...
        <div className="col-span-12 lg:col-span-4">
          <CardBox>
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/dashboards/echoes/categories" className="text-primary text-sm font-semibold">Manage</Link>
            </div>
            {by_category.length ? (
              <div className="flex flex-col gap-2.5">
                {by_category.map((c) => (
        ...
        <div className="col-span-12 lg:col-span-4">
          <CardBox>
            <h5 className="card-title mb-3">Top echoes</h5>
            {top_echoes.length ? (
              <div className="flex flex-col divide-y divide-border dark:divide-darkborder">
                {top_echoes.map((e) => <EchoRow key={e.uuid} e={e} metric="listeners" />)}
              </div>
            ) : <div className="py-4 text-center text-sm text-darklink">No ended echoes yet.</div>}
          </CardBox>
        </div>
      </div>
    </div>
...
export default EchoOverviewContent;
